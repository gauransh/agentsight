...
:root{{--font-sans:"Inter Variable",Inter,"SF Pro Display",system-ui,-apple-system,"Segoe UI",sans-serif;--font-mono:ui-monospace,"SF Mono",SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace;font-family:var(--font-sans);background:var(--bg);color:var(--ink)}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg)}}.artifact{{width:1264px;min-height:865px;padding:24px 32px;background:var(--bg);transition:box-shadow .12s}}.artifact.commit-flash{{box-shadow:inset 0 0 0 2px var(--flash),0 0 30px var(--flash-glow)}}.header{{display:flex;justify-content:space-between;gap:24px;border-bottom:1px solid var(--hair);padding-bottom:14px}}.eyebrow,.mode{{font:11px var(--font-mono);color:var(--accent);letter-spacing:.12em;text-transform:uppercase}}.header h1{{font-size:28px;margin:6px 0}}.header p{{font-size:12px;color:var(--ink-dim);margin:0}}.mode{{color:var(--ink-mode);border:1px solid var(--hair);padding:7px 9px;border-radius:99px;align-self:flex-start}}.visual{{width:1200px;height:675px;margin-top:14px}}.timeline{{display:grid;grid-template-columns:42px 1fr 190px;gap:12px;align-items:center;border-top:1px solid var(--hair);padding:14px 0 8px}}.timeline button{{width:36px;height:36px;border-radius:50%;border:1px solid var(--accent-border);background:var(--accent-bg);color:var(--accent)}}.timeline input{{width:100%;accent-color:var(--accent)}}.timeline output{{font:10px var(--font-mono);color:var(--ink-cursor);text-align:right}}.legend{{display:flex;gap:18px;font:10px var(--font-mono);color:var(--ink-dim)}}.legend i{{display:inline-block;width:13px;height:3px;margin-right:5px;vertical-align:middle}}.footer{{margin-top:8px;color:var(--ink-footer);font:10px var(--font-mono)}}
